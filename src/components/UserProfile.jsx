...
const UserProfile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/users/${batch}/profile`,
        {
          headers: {
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">
                  Welcome, {profile.name || profile.Handle}
                </h1>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {restrictions.nextHandleUpdate && (
                <div className="flex items-center space-x-2">
                  <Clock className="w-5 h-5 text-zinc-500" />
                  <span className="text-sm text-zinc-400">
                    Next handle update: {formatTimestamp(restrictions.nextHandleUpdate)}
                  </span>
                </div>
              )}
            </div>
  ...
  {[
    { key: "LeetCodeHandle", name: "LeetCode", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v13/icons/leetcode.svg" },
    { key: "CodeforcesHandle", name: "Codeforces", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v13/icons/codeforces.svg" },
    { key: "CodeChefHandle", name: "CodeChef", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v13/icons/codechef.svg" },
    { key: "HackerRankHandle", name: "HackerRank", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v13/icons/hackerrank.svg" },
    { key: "GeeksForGeeksHandle", name: "GeeksForGeeks", icon: "https://cdn.jsdelivr.net/npm/simple-icons@v13/icons/geeksforgeeks.svg" }
  ].map((platform) => (
    <div
      key={platform.key}
      className="p-6 rounded-2xl bg-white/5 shadow-md border border-zinc-200/20 
                 hover:shadow-xl hover:border-zinc-300/40 hover:scale-105 
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-zinc-200 font-semibold">
          {parseInt(
            profile[platform.key]?.rating ||
              profile[platform.key]?.codingScore ||
              0
          )}
        </span>
      </div>
      ...
      {editing ? (
        <input
          ...
          className="w-full px-3 py-2 border border-zinc-700 rounded-lg text-white placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
        />
      ) : (
      ...
      <div className="mt-3 text-sm text-zinc-400">
        ...
        <span className="font-medium text-zinc-200">
          {parseInt(
            profile[platform.key]?.contests ||
              profile[platform.key]?.contestsAttended ||
              0
          )}
        </span>
      </div>
